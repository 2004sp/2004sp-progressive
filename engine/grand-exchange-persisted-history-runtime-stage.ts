...
function patchOpcodeMap() {
    ...
    const marker = "    ['MIDI_LENGTH', ScriptOpcode.MIDI_LENGTH],";
    if (!source.includes(marker)) throw new Error('Cannot find server opcode map insertion point');
    const mappings = [
    ...
    fs.writeFileSync(SCRIPT_OPCODE_PATH, source.replace(marker, `${marker}\n${mappings}`), 'utf8');
}

function patchServerOps() {
    let source = fs.readFileSync(SERVER_OPS_PATH, 'utf8').replace(/\r/g, '');
    if (source.includes('GE_HISTORY_OPCODE')) throw new Error('GE history handlers already staged');
    ...
    const helpers = `const GE_HISTORY_OPCODE = ${JSON.stringify(OPCODE, null, 4)} as const;\n\nfunction geHistoryPlayer(state: ScriptState) {\n    return state.self instanceof Player ? state.self : null;\n}\n\n`;
    source = source.replace(mapStart, `${helpers}${mapStart}`);
...
function validateRuntimeBridge() {
    ...
    for (const [name, value] of Object.entries({ GE_HISTORY_RECORD: OPCODE.RECORD, GE_HISTORY_SET_STATUS: OPCODE.SET_STATUS, GE_HISTORY_EXISTS: OPCODE.EXISTS, GE_HISTORY_ITEM: OPCODE.ITEM, GE_HISTORY_INT: OPCODE.INT_FIELD, GE_HISTORY_TIMESTAMP: OPCODE.TIMESTAMP })) {
        if (!opcodes.includes(`['${name}', ${value}]`)) throw new Error(`Missing staged opcode ${name}`);
    }
    for (const name of ['RECORD', 'SET_STATUS', 'EXISTS', 'ITEM', 'INT_FIELD', 'TIMESTAMP'] as const) {
        if (!handlers.includes(`[GE_HISTORY_OPCODE.${name}]`)) throw new Error(`Missing staged GE history handler ${name}`);
    }
