...
export default class ClientCheatHandler extends ClientGameMessageHandler<ClientCheat> {
    private parseTeleport(args: string[], player: Player): { x: number; z: number; level: number } | null {
        if (args.length < 1) {
            return null;
        }

        if (args[0] === 'up' || args[0] === 'down') {
            const level = player.level + (args[0] === 'up' ? 1 : -1);
            if (level < 0 || level > 3) {
                return null;
            }

            return { x: player.x, z: player.z, level };
        }
        ...
        if (args[0].includes(',')) {
            ...
            const level = tryParseInt(coord[0], -1);
            const mx = tryParseInt(coord[1], -1);
            const mz = tryParseInt(coord[2], -1);
            ...
            return { x: (mx << 6) + lx, z: (mz << 6) + lz, level };
        }
        ...
        const x = tryParseInt(args[0], -1);
        const z = tryParseInt(args[1], -1);
        const level = tryParseInt(args[2], player.level);

        if (level < 0 || level > 3 || x < 0 || x > 0x3fff || z < 0 || z > 0x3fff) {
            return null;
        }
    ...
    private teleportPlayer(args: string[], player: Player): boolean {
        const coord = this.parseTeleport(args, player);
        if (!coord) {
            return false;
        }
        ...
        player.unsetMapFlag();
        player.teleJump(coord.x, coord.z, coord.level);
    ...
    handle(message: ClientCheat, player: Player): boolean {
        ...
        if (player.staffModLevel >= 2) {
            ...
            if (cmd === 'getcoord') {
                ...
                // ::tele level,mx,mz[,lx,lz] uses authentic map-square coords.
                // ::tele x z level is a convenience for absolute world coords.
                return this.teleportPlayer(args, player);
